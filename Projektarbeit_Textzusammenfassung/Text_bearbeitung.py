...
class Text_bearbeiten:
    ...
    def only_words(self)->list:
        ...
        return [word.lower() for word in words if (w := word.lower()) and is_valid_token(w) and w not in punctuation and w not in set_stopwords]
    ...
    def generate_summary(self, sentence_count: dict[str, float]) -> str:
        ...
        number_of_sentences = max(1, round(0.2 * len(sentence_count)))
